      ...
      <aside className="hidden lg:flex flex-col border-r border-line bg-ink-50/30">
        ...
        <div className="flex-1 overflow-y-auto">
          {(!convs || convs.length === 0) && (
            <div className="px-6 py-10 text-sm font-light text-bone-muted">
              No sessions yet.
            </div>
          )}
          <ul>
            {convs?.map((c) => (
              <li key={c.id}>
                <Link
                  href={`/mirror/${c.id}`}
                  className="block px-6 py-4 border-b border-line hover:bg-bone/[0.02] transition-colors"
                >
                  <div className="text-sm font-light text-bone line-clamp-1">
                    {c.title ?? "Untitled"}
                  </div>
